...
class SirepoFlyer(BlueskyFlyer):
    def __init__(self, sim_id, server_name, params_to_change, sim_code='srw', copy_count=5, watch_name='Watchpoint', run_parallel=True):
        super().__init__()
    ...
    def kickoff(self):
        ...
        for param in self.params_to_change:
            ...
            # want to be able to change any parameters to any value
    ...
    def describe_collect(self):
        ...
        for i in range(len(elem_name)):
            for j in range(len(curr_param[i])):
                return_dict[self.name][f'{self.name}_{elem_name[i]}_{curr_param[i][j]}'] = {'source': f'{self.name}_{elem_name[i]}_{curr_param[i][j]}',
                                     'dtype': 'number',
                                     'shape': []}
        return return_dict
        ...
        for i, datum_id in enumerate(self._datum_ids):
            ...
            for ii in range(len(elem_name)):
                for jj in range(len(curr_param[ii])):
                    data[f'{self.name}_{elem_name[ii]}_{curr_param[ii][jj]}'] = params_to_change[i][elem_name[ii]][curr_param[ii][jj]]
                    # params_to_change[2][elem_name[ii]][curr_param[ii][jj]]
            yield {'data': data,
...
if __name__ == '__main__':
    ...
    sirepo_flyer = SirepoFlyer(sim_id='87XJ4oEb', server_name='http://10.10.10.10:8000', params_to_change=params_to_change,
                               watch_name='W60')
